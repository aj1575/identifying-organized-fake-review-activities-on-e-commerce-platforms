...
print("RUNNING USER TABLE + DGIM + BLOOM FILTER")
...
class DGIM:
    def __init__(self):
        # Each bucket is stored as (size, end_time)
        # Newest bucket first
        self.buckets = []
        self.current_time = 0
    ...
    def _compress_buckets(self):
        ...
        while changed:
            ...
            for bucket_size, indices in size_to_indices.items():
                if len(indices) > 2:
                    # newest first, so last two are oldest
...
# =========================================================
# 9.5 K-ANONYMITY (ADD ANONYMOUS USER COLUMN)
# =========================================================
...
# Initial placeholder; bloom query computes this properly later
final_df = final_df.withColumn(
...
def process_bloom_filter(batch_df, batch_id):
    ...
    # only users whose current 5-min window is a burst
    ...
    # safer than show(50, truncate=False)
...
query1 = final_df.writeStream \
    ...
    .option("numRows", 20) \
    .queryName("user_level_table") \
    .start()


# =========================================================
# 14. QUERY 2: DGIM OUTPUT
# =========================================================
query2 = clean_df.writeStream \
    .foreachBatch(process_dgim) \
    .outputMode("append") \
    .queryName("dgim_output") \
    .start()


# =========================================================
# 15. QUERY 3: BLOOM FILTER OUTPUT
# =========================================================
query3 = final_df.writeStream \
    .foreachBatch(process_bloom_filter) \
    .outputMode("complete") \
    .queryName("bloom_filter_output") \
    .start()
...
def process_lsh(batch_df, batch_id):
    ...
    # Keep this separate; does not change your existing logic
    ...
    # Optional: focus only on low-rating reviews for suspicious pattern matching
    ...
    # Add row id for self-join filtering
    ...
    # Fit LSH model inside this batch only
...
# =========================================================
# 19. QUERY 4: LSH OUTPUT
# =========================================================
query4 = clean_df.writeStream \
    .foreachBatch(process_lsh) \
    .outputMode("append") \
    .queryName("lsh_output") \
    .start()


# =========================================================
# 16. KEEP STREAMS RUNNING
# =========================================================
